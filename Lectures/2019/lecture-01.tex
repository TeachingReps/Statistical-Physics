...
Mathematically, E can be seen as a linear operator from the space of real functions on X to the set of real numbers. Thus,
\EQ{
...
\end{exmp}
\begin{defn} 
A continuous random variable, $X$, taking values on the set $\sX = \R^d$ is
defined by its cumulative distribution function $P(X \le x)$, where $X \le x$ is used to denote
$X_i \le x_i$ for $i = 1, \dots, d$. For such a r.v., the probability measure with respect to the
infinitesimal element dx is denoted by dpX (x). For a measurable event A ? X , this gives ??
x?A
??
P (X ? x) = x?a b?a
?0 if x < a
...
dpX(x) = I(x ? A)dpX(x),
x?A
where the indicator function I(s) is 1 if the logical statement s is true and 0 otherwise. If
pX admits a density, with respect to Lebesgue measure, then it will be denoted by pX(x). In this case, we can write
??
P(X ? A) =
\end{defn}
\begin{exmp}
If X is a continuous random variable defined, for a, b ? R with a < b, by
$p_X(x)dx = I(x ? A)pX(x)dx. ?
 then it is uniform on [a, b] and its density is given by pX (x) =
Definition 9. The expected value and variance of a function f : Rd ? R of a continuous
random variable X ? Rd are given by ?
1 I(x ? [a, b]). b?a
 Ef = E[f(X)] =
Varf =Var(f(X))=E[(f(X)?E[f(X)])]=E??f(X)2???E[f(X)]2.
\end{exmp}
\begin{exmp} 
If X is a continuous random variable that is uniform on [a, b], then its mean and variance are given by
a
f(x)dpX(x)
?b x b2?a2 b+a E[X]= b?adx= 2(b?a) = 2
??? b x2 ?? ??b+a??2 b3 ?a3
   Var (f (X )) = b ? a dx ? 2 = 3(b ? a) ? a
??b+a??2 1 2 2 = 12 (b ? a) .
\end{exmp}
     
II. ENTROPY
In statistical mechanics, the entropy is proportional to the logarithm of the number of resolvable microstates associated with a macrostate. In classical mechanics, this quantity contains an arbitrary additive constant associated with the size a of a microstate that is considered resolvable. In quantum mechanics, there is a natural limit to resolvability and this constant is related to the Planck constant. For random variables, Shannon chose the following definition which is similar in spirit.
Definition 11. The entropy (in bits) of a discrete random variable X with probability distribution p(x) is denoted
??1??1??
p(x)log2p(x)=E log2p(X) ,
where 0 log2 0 = 0 by continuity. The notation H (p) is used to denote H (X ) when X ? p(x). When there is no ambiguity, H will be used instead of H (X). The unit of entropy is determined by the base of the logarithm with base-2 resulting in ?bits? and the natural log (i.e., base-e) resulting in ?nats?.
Remark 12. Roughly speaking, the entropy H (X) measures the uncertainty in the random variable X.
Example 13. If X is uniform, then p(x) = 1/|X | and ??1??
H (X) = E log2 1/|X| = log2 |X|.
Choosing |X | = 2, we see that a uniform random bit has exactly log2 2 = 1 bit of entropy.
Example 14. Let X be a binary r.v. defined by p(0) = 1?q and p(1) = q. In this case, we have
H(X)=H(q)?qlog 1+(1?q)log 1 , 2q 21?q
where H(q) is called the binary entropy function. This function is concave and symmetric about q = 1/2. It also satisfies H(0) = H(1) = 0 and H(1/2) = 1.
H(X)?
3
  x?X
   
                      Using Stirling?s formula, n! = ?2?n ?? n ??n ??1 + O ?? 1 ????, we see that en
qn
1
4
 0.8
...
0 0.2 0.4 0.6 0.8 1
q
Figure 1: The binary entropy function H(q).
Example 15. The number of length-n binary sequences with exactly qn ones is given by?? n ??.
   ??n?? n!
qn = =
(n ? qn)!(qn)!
?2?n??n??n ??1+O??1???? en
...
1 nH(q)?? ?? 1 ????
   Remark 16. This shows that the binary entropy determines the exponential growth rate of the number of binary sequences with a fixed fraction of ones. In fact, this is a fundamental property of the entropy. More generally, we will see that the entropy H (X) is the expo- nential growth rate of the number of length-n sequences (i.e., there are roughly 2nH(X)such sequences) where the fraction of x?s converges to np(x). This also implies that nH (X) is essentially equal to the minimum number of binary digits required to index all length-n sequences of this type.
Lemma 17. Basic properties of entropy:
1. (non-negativity of entropy) H (X) ? 0 with equality iff X is constant.
H(q)

Proof: log 1 ? 0 and, if X is not constant, there is an x with p(x ) ? (0,1). Thus,
 2 p(x)
