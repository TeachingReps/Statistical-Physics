...
Studying the REM thus consists of examining a probabilistic object over levels that are them- selves probabilistically distributed. Each realization of disorder is a sample, but calculating the properties of a specific sample with finite N is typically very difficult?it corresponds to finding an optimal distribution of a particular instance of a problem. We will instead average over these samples, which we denote E(·), and is typically a much easier calculation to perform. Because a lot of systems have thermodynamic potentials X that self-average, i.e., ?concentrate? as N increases, i.e., for any tolerance ? > 0
?? ???? X N ?? X N ?? ???? ??
lim P ?? ?E ???? =0, (3)
