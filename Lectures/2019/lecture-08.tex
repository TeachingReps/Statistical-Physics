...
E(\sigma) = - \sum_{(i,j) \in F} \sigma_i \sigma_j - B \sum_{i \in [L]^d} \sigma_i.
}
...
\begin{defn}[Expected spin] 
 ...
 \EQ{
 \inner{\sigma_i} = \sum_{\sigma \in \sX^N}\mu_\beta(\sigma)\sigma_i.
 }
 \end{defn}
 We can write the scaled energy function for the ferromagnetic Ising model as
\EQ{
-\beta E(\sigma) = \beta \sum_{(i, j) \in F}\sigma_{i}\sigma_{j} + x \sum_{i\in [L]^d}\sigma_{i}. 
}
\begin{itemize}
...
\EQ{
\mu_\beta(\sigma) = \prod_{i}\mu_\beta(\sigma_i), \text { where } \mu_\beta(\sigma_i) = \frac{e^{x\sigma_i}}{e^{x}+e^{-x}}. 
}
